...
const addIdentation = (code: string, indentation: number): string => {
  return code
    .split("\n")
    .map((line, i) => (i > 0 ? "   ".repeat(indentation) + line : line))
    .join("\n");
};
 ...
 * @returns An array with the opening and closing span tags
 */
const getSyntaxHighlightTags = (
  type: "comment" | "keyword" | "literal" | "string" | "type"
): string[] => {
  return getSpanTags(`class="${type}"`);
};

/**
 * @brief Normalizes a filepath, so that it only contains forward slashes.
 * @param filepath
 * @returns
 */
